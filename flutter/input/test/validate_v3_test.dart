...
main(){
  test("normal response", (){
    var response = """{"results":[{"columns":["n.title","n","date()","ID(n)"],"data":[{"row":["The Matrix",{"tagline":"Welcome to the Real World","title":"The Matrix","released":1999},"2021-08-03",0],"meta":[null,{"id":0,"type":"node","deleted":false},{"type":"date"},null]}]}],"errors":[]}""";

    var validated = validateV3Response(response);
    expect(validated, isNotNull);
    expect(validated.error, equals(''));
    expect(validated.returnValues.length, equals(4));
